  ...
  return (
    <footer className="relative overflow-hidden bg-hall text-cream">
      ...
      <div className="relative flex h-1.5" aria-hidden="true">
        {HOUSES.map((house) => (
          <HouseScarf key={house.id} house={house} className="flex-1" />
        ))}
      </div>
      <div className="relative mx-auto grid max-w-[1240px] gap-12 px-5 py-16 md:grid-cols-12 md:px-8">
        <div className="md:col-span-4">
          ...
          <p className="mt-4 max-w-sm text-sm leading-relaxed text-cream/65">
            An SAT school in Tashkent for students who need a funded seat. Four houses. One
            method. Taught in English.
          </p>
        </div>
        ...
        <div className="md:col-span-3">
          <p className="text-[0.72rem] tracking-[0.14em] text-sage uppercase">Houses</p>
          <ul className="mt-4 space-y-2">
            {HOUSES.map((house) => (
              <li key={house.id} className="flex items-center gap-2.5 text-sm text-cream/80">
                <span
                ...
                <span
                  ...
                  style={{ color: house.id === "amberfell" ? house.hex : house.metal }}
                >
                  {house.name}
                </span>
                <span className="text-cream/55">· {house.track}</span>
              </li>
            ))}
          </ul>
        </div>
      ...
      <div className="relative mx-auto max-w-[1240px] px-5 md:px-8">
        <div className="flex justify-center pb-6">
          <Crest className="size-16" ink={CREAM} />
        </div>
        <p
          aria-hidden="true"
